...
- [ ] **Partial unique `idx_receipts_payment_id` (`WHERE provider_payment_id IS NOT NULL`)** (миграция 015) — race-защита от дубликатов receipt'ов при ретраях webhook'а CloudPayments. На реальном PG проверяем: два конкурентных INSERT с одним `TransactionId` → первый проходит, второй получает SQLSTATE `23505`. В webhook-handler'ах (`/webhook/cloudpayments/pay`, `/recurrent`, `/fail`) есть проактивная SELECT-проверка по `provider_payment_id`, но UNIQUE — гарантия consistency на случай race-окна (~10мс между SELECT и INSERT). В test/helpers.js этот индекс дропается из-за pg-mem partial-WHERE quirk.
- [ ] **Partial index `idx_receipts_failed` (`WHERE is_failed = true`)** (миграция 015) — ускоряет lookup failed-receipts саппортом и UI «список неуспешных списаний». На реальном PG проверяем: при ≥1000 receipts (99% успешных, 1% failed) `EXPLAIN ANALYZE SELECT * FROM receipts WHERE subscription_id=$1 AND is_failed=true ORDER BY created_at DESC` использует partial-индекс. В test/helpers.js дропается из-за pg-mem partial-WHERE quirk.
- [ ] **`ALTER TABLE … DROP COLUMN IF EXISTS` требует существования таблицы** (rollback 015). `IF EXISTS` в этом синтаксисе относится только к колонке, а не к таблице. Если таблица не существует — `relation does not exist`. В rollback 015 ALTER-statements обёрнуты в `DO $$ … EXISTS (SELECT … FROM information_schema.tables) … $$` блок для совместимости со сценарием «частичный rollback при отсутствующих receipts/subscriptions». pg-mem стрипает DO-блоки. На реальном PG проверяем: применяем 001-014 → выполняем `rollback/015_rollback.sql` (несмотря на отсутствие is_failed колонки) → ошибки быть не должно.
