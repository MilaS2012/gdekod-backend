...
| `private-api/` | Auth + email-привязка + сессии + подписки (`/auth/*`, `/auth/email/*`, `/auth/banner/dismiss`, `/auth/sessions`, `/auth/logout-all`, `/subscription/*`), `requireUser` middleware, rate-limit, mask-pii, billing-config, mock-cron | 🟡 6.1–6.6 готовы, 6.7–6.11 в работе | 353 / 353 (`node:test`), coverage 93.43% |
...
| 6.7–6.11 | Приватный API: аккаунт, парсер, 152-ФЗ, support, финал | `private-api/` | — |
