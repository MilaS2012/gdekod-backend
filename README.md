...
| `private-api/` | Auth + email-привязка + сессии + подписки + аккаунт + промокоды (`/auth/*`, `/auth/email/*`, `/auth/banner/dismiss`, `/auth/sessions`, `/auth/logout-all`, `/subscription/*`, `/account/*`, `/coupons/{id}/*`), `requireUser` middleware, rate-limit, mask-pii, billing-config, mock-cron, coupon-config | 🟡 6.1–6.7 готовы, 6.8–6.11 в работе | 420 / 420 (`node:test`), coverage 93.13% |
...
| 6.8–6.11 | Приватный API: парсер-эндпоинты, 152-ФЗ, support, финал | `private-api/` | — |
