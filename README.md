...
| `private-api/` | Auth (`/auth/start`, `/auth/verify`, `/auth/login-magic`), `requireUser` middleware, rate-limit, mask-pii | 🟡 6.1–6.3 готовы, 6.4–6.11 в работе | 221 / 221 (`node:test`), coverage 94.51% |
...
| 6.4–6.11 | Приватный API: email, сессии, подписки, аккаунт, парсер, 152-ФЗ, support, финал | `private-api/` | — |
...
- `POST /api/auth/start` → channel (`flash_call` | `magic_link`)
