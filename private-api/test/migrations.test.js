...
test('migrate.js listMigrations — все миграции в числовом порядке', async () => {
    ...
    assert.ok(files.length >= 11, `ожидаем минимум 11 миграций, получено ${files.length}`);
    assert.equal(files[0], '001_init.sql');
    assert.equal(files.at(-1), '011_user_profile_fields.sql');
...
test('migrate.js listRollbacks — все rollback в обратном порядке', async () => {
    ...
    assert.equal(files[0], '011_rollback.sql');     // от старшего к младшему
