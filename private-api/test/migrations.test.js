...
test('migrate.js listMigrations — все миграции в числовом порядке', async () => {
    ...
    assert.ok(files.length >= 9, `ожидаем минимум 9 миграций, получено ${files.length}`);
    assert.equal(files[0], '001_init.sql');
    assert.equal(files.at(-1), '009_receipts.sql');
...
test('migrate.js listRollbacks — все rollback в обратном порядке', async () => {
    ...
    assert.equal(files[0], '009_rollback.sql');     // от старшего к младшему
