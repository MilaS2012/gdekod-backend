...
//
//  12. idx_users_deletion_scheduled (миграция 014) — partial WHERE
//      deletion_scheduled_at IS NOT NULL AND deletion_completed_at IS NULL.
//      Та же проблема: ломает SELECT'ы с теми же условиями. Дропается в
//      test/helpers.js. На реальном PG ускоряет processScheduledDeletions.
//
...
// -----------------------------------------------------------------------------
// Группа 013 — support_tickets и events_log
// -----------------------------------------------------------------------------
...
test('013: создаёт таблицы support_tickets и events_log', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    assert.equal(tableExists(db, 'private_data', 'support_tickets'), true);
    assert.equal(tableExists(db, 'private_data', 'events_log'),      true);
});

test('013: support_tickets имеет все колонки спеки', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    for (const c of ['id', 'user_id', 'category', 'subject', 'message', 'status',
                     'created_at', 'updated_at', 'closed_at',
                     'contact_phone', 'contact_email']) {
        assert.ok(columnExists(db, 'private_data', 'support_tickets', c),
                  `support_tickets.${c} missing`);
    }
});

test('013: support_tickets CHECK category — недопустимое значение отвергается', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    const u = db.public.one(
        `INSERT INTO private_data.users (phone) VALUES ('+79261111111') RETURNING id`
    );
    assert.throws(
        () => db.public.none(
            `INSERT INTO private_data.support_tickets
               (user_id, category, subject, message, contact_phone)
             VALUES ('${u.id}', 'malformed', 'subj', 'message text 1234', '+79261111111')`
        ),
        /check/i,
    );
});

test('013: support_tickets CHECK status — недопустимое значение отвергается', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    const u = db.public.one(
        `INSERT INTO private_data.users (phone) VALUES ('+79261111111') RETURNING id`
    ...
    assert.throws(
        () => db.public.none(
            `INSERT INTO private_data.support_tickets
               (user_id, category, subject, message, status, contact_phone)
             VALUES ('${u.id}', 'other', 'subj', 'message text 1234', 'unknown', '+79261111111')`
        ),
        /check/i,
    );
});

test('013: support_tickets FK user_id — несуществующий user отвергается', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    assert.throws(
        () => db.public.none(
            `INSERT INTO private_data.support_tickets
               (user_id, category, subject, message, contact_phone)
             VALUES ('00000000-0000-0000-0000-000000000000', 'other', 'subj',
                     'message text 1234', '+79261111111')`
        ),
        /foreign|reference/i,
    );
});

test('013: support_tickets ON DELETE CASCADE — удаление user уносит тикеты', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    const u = db.public.one(
        `INSERT INTO private_data.users (phone) VALUES ('+79261111111') RETURNING id`
    ...
    db.public.none(
        `INSERT INTO private_data.support_tickets
           (user_id, category, subject, message, contact_phone)
         VALUES ('${u.id}', 'other', 'subj', 'message text 1234', '+79261111111')`
    );
    db.public.none(`DELETE FROM private_data.users WHERE id = '${u.id}'`);
    const c = db.public.one(
        `SELECT count(*)::int AS c FROM private_data.support_tickets`
    );
    assert.equal(c.c, 0);
});

test('013: events_log имеет все колонки спеки', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    for (const c of ['id', 'user_id', 'event_type', 'payload',
                     'coupon_id', 'merchant_id',
                     'ip_address', 'user_agent_hash', 'created_at']) {
        assert.ok(columnExists(db, 'private_data', 'events_log', c),
                  `events_log.${c} missing`);
    }
});

test('013: events_log ON DELETE SET NULL — удаление user сохраняет событие', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    const u = db.public.one(
        `INSERT INTO private_data.users (phone) VALUES ('+79261111111') RETURNING id`
    ...
    db.public.none(
        `INSERT INTO private_data.events_log (user_id, event_type)
         VALUES ('${u.id}', 'coupon_viewed')`
    );
    db.public.none(`DELETE FROM private_data.users WHERE id = '${u.id}'`);
    const c = db.public.one(
        `SELECT count(*)::int AS c, count(user_id)::int AS u
           FROM private_data.events_log`
    );
    assert.equal(c.c, 1, 'событие должно остаться');
    assert.equal(c.u, 0, 'user_id должен стать NULL');
});

test('013: events_log payload как JSONB сохраняется и читается', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    const u = db.public.one(
        `INSERT INTO private_data.users (phone) VALUES ('+79261111111') RETURNING id`
    ...
    db.public.none(
        `INSERT INTO private_data.events_log (user_id, event_type, payload)
         VALUES ('${u.id}', 'search_performed', '{"q":"тест","page":2}'::jsonb)`
    );
    const row = db.public.one(
        `SELECT payload FROM private_data.events_log LIMIT 1`
    );
    // pg-mem возвращает JSONB как объект.
    assert.equal(row.payload.q, 'тест');
    assert.equal(row.payload.page, 2);
});

// -----------------------------------------------------------------------------
// Группа 014 — soft-delete + account_deletion_otp_codes
// -----------------------------------------------------------------------------

test('014: добавляет deletion_scheduled_at и deletion_completed_at в users', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    for (const c of ['deletion_requested_at', 'deletion_scheduled_at',
                     'deletion_completed_at']) {
        assert.ok(columnExists(db, 'private_data', 'users', c),
                  `users.${c} missing`);
    }
});

test('014: создаёт таблицу account_deletion_otp_codes', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    assert.equal(
        tableExists(db, 'private_data', 'account_deletion_otp_codes'),
        true,
    );
});

test('014: account_deletion_otp_codes имеет все колонки спеки', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    for (const c of ['id', 'user_id', 'code_hash', 'expires_at', 'used_at',
                     'attempts_count', 'ip_address', 'created_at']) {
        assert.ok(columnExists(db, 'private_data', 'account_deletion_otp_codes', c),
                  `account_deletion_otp_codes.${c} missing`);
    }
});

test('014: account_deletion_otp_codes FK user_id — несуществующий user отвергается', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    assert.throws(
        () => db.public.none(
            `INSERT INTO private_data.account_deletion_otp_codes
             ...
             VALUES ('00000000-0000-0000-0000-000000000000', 'h',
...
test('014: account_deletion_otp_codes ON DELETE CASCADE — удаление user уносит OTP', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    const u = db.public.one(
        `INSERT INTO private_data.users (phone) VALUES ('+79261111111') RETURNING id`
    ...
    db.public.none(
        `INSERT INTO private_data.account_deletion_otp_codes
           (user_id, code_hash, expires_at)
         VALUES ('${u.id}', 'h', now() + interval '5 minutes')`
    );
    db.public.none(`DELETE FROM private_data.users WHERE id = '${u.id}'`);
    const c = db.public.one(
        `SELECT count(*)::int AS c FROM private_data.account_deletion_otp_codes`
    );
    assert.equal(c.c, 0);
});

test('014: deletion_scheduled_at и deletion_completed_at — оба nullable по умолчанию', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    const u = db.public.one(
        `INSERT INTO private_data.users (phone) VALUES ('+79261111111')
         RETURNING deletion_scheduled_at, deletion_completed_at`
    );
...
// -----------------------------------------------------------------------------
// Группа 015 — CloudPayments fields
// -----------------------------------------------------------------------------

test('015: добавляет last_charge_at в subscriptions', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    assert.ok(columnExists(db, 'private_data', 'subscriptions', 'last_charge_at'),
              'subscriptions.last_charge_at missing');
});

test('015: добавляет is_failed в receipts с DEFAULT false', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    assert.ok(columnExists(db, 'private_data', 'receipts', 'is_failed'),
              'receipts.is_failed missing');

    // DEFAULT false: создаём user + subscription + receipt без указания is_failed
    // (db.public.one не поддерживает $1 параметры — конкатенируем UUID).
    const u = db.public.one(
        `INSERT INTO private_data.users (phone) VALUES ('+79262222222') RETURNING id`
    );
    const sub = db.public.one(
        `INSERT INTO private_data.subscriptions
         ...
         VALUES ('${u.id}', 'monthly_499', 'cloudpayments_card', 'pending', 49900,
    ...
    const r = db.public.one(
        `INSERT INTO private_data.receipts
         ...
         VALUES ('${u.id}', '${sub.id}', 49900, 'RUB', 'cloudpayments_card',
                 now(), now() + interval '30 days')
         RETURNING is_failed`
    );
    assert.equal(r.is_failed, false, 'DEFAULT false не сработал');
});

test('015: last_charge_at nullable по умолчанию', async () => {
    const db = freshDb();
    await applyAllMigrations(db);
    const u = db.public.one(
        `INSERT INTO private_data.users (phone) VALUES ('+79263333333') RETURNING id`
    );
    const sub = db.public.one(
        `INSERT INTO private_data.subscriptions
           (user_id, tariff, provider, status, amount_kopecks,
            activated_at, expires_at, next_charge_at)
         VALUES ('${u.id}', 'monthly_499', 'cloudpayments_card', 'active', 49900,
                 now(), now() + interval '30 days', now() + interval '30 days')
         RETURNING last_charge_at`
    );
    assert.equal(sub.last_charge_at, null);
});

// -----------------------------------------------------------------------------
// Группа 016 — top-20 merchants seed
// -----------------------------------------------------------------------------

test('016: создаёт UNIQUE-индекс на domain и вставляет 20 магазинов', async () => {
    const db = freshDb();
    await applyAllMigrations(db);

    const row = db.public.one(
        `SELECT count(*)::int AS c FROM public_data.merchants`
    );
    assert.ok(row.c >= 20, `ожидаем минимум 20 магазинов, получено ${row.c}`);
});

test('016: конкретные домены присутствуют (wildberries, ozon, mvideo)', async () => {
    const db = freshDb();
    await applyAllMigrations(db);

    for (const domain of ['wildberries.ru', 'ozon.ru', 'mvideo.ru', 'lamoda.ru', 'adidas.ru']) {
        const row = db.public.one(
            `SELECT count(*)::int AS c FROM public_data.merchants WHERE domain = '${domain}'`
        );
        assert.equal(row.c, 1, `домен ${domain} не найден`);
    }
});

test('016: повторное применение идемпотентно (ON CONFLICT DO NOTHING)', async () => {
    const db = freshDb();
    await applyAllMigrations(db);

    const before = db.public.one(
        `SELECT count(*)::int AS c FROM public_data.merchants`
    ).c;

    // Применяем 016 второй раз — количество не должно вырасти.
    // pg-mem поддерживает ON CONFLICT с UNIQUE-индексом → строгий no-op.
    const sql016 = stripUnsupportedForPgMem(await readMigrationFile('016_top_merchants_seed.sql'));
    assert.doesNotThrow(() => db.public.none(sql016));

    const after = db.public.one(
        `SELECT count(*)::int AS c FROM public_data.merchants`
    ).c;
    assert.equal(after, before, 'повторный INSERT не должен добавлять строки');
});
...
test('migrate.js listMigrations — все миграции в числовом порядке', async () => {
    ...
    assert.ok(files.length >= 16, `ожидаем минимум 16 миграций, получено ${files.length}`);
    assert.equal(files[0], '001_init.sql');
    assert.equal(files.at(-1), '016_top_merchants_seed.sql');
...
test('migrate.js listRollbacks — все rollback в обратном порядке', async () => {
    ...
    assert.equal(files[0], '016_rollback.sql');     // от старшего к младшему
