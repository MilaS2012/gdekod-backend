...
test('migrate.js listMigrations — все миграции в числовом порядке', async () => {
    ...
    assert.ok(files.length >= 13, `ожидаем минимум 13 миграций, получено ${files.length}`);
    assert.equal(files[0], '001_init.sql');
    assert.equal(files.at(-1), '013_support_and_events.sql');
...
test('migrate.js listRollbacks — все rollback в обратном порядке', async () => {
    ...
    assert.equal(files[0], '013_rollback.sql');     // от старшего к младшему
