...
test('migrate.js listMigrations — все миграции в числовом порядке', async () => {
    ...
    assert.ok(files.length >= 15, `ожидаем минимум 15 миграций, получено ${files.length}`);
    assert.equal(files[0], '001_init.sql');
    assert.equal(files.at(-1), '015_cloudpayments_fields.sql');
...
test('migrate.js listRollbacks — все rollback в обратном порядке', async () => {
    ...
    assert.equal(files[0], '015_rollback.sql');     // от старшего к младшему
